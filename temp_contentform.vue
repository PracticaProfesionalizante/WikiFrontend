    ...
    <div class="modal-container" @click.stop>
          ...
          <div class="wizard-title-section">
            ...
            <div class="wizard-text">
              ...
              <p class="wizard-subtitle">
                {{
                  isEditing
                    ? 'Modifica la configuraci├│n del documento existente'
      ...
      <div v-if="!loading" class="wizard-body">
        <form @submit.prevent="handleSubmit" class="wizard-form">
            ...
            <div class="form-fields">
              ...
              <div class="form-field">
                ...
                >
                  ...
                  <option value="TYPE_URL">­ƒöù Enlace/URL</option>
                  <option value="TYPE_TEXT">­ƒôä Documento de Texto</option>
                  <option value="TYPE_PDF">­ƒôï Documento PDF</option>
                </select>
                  ...
                  <div class="file-upload-area" :class="{ 'has-file': pdfFile }">
                    ...
                    <div class="upload-content">
                      <div v-if="!pdfFile" class="upload-placeholder">
                        ...
                        <span class="upload-hint">Solo archivos PDF (m├íximo 10MB)</span>
                      </div>
          ...
          <!-- Paso 3: Configuraci├│n y Permisos -->
          <div v-show="currentWizardStep === 3" class="wizard-step-content">
            <div class="step-header">
              ...
              <p class="step-description">Define qui├®n puede acceder a este documento</p>
            </div>
          ...
          <div v-show="currentWizardStep === 4" class="wizard-step-content">
            <div class="step-header">
              ...
              <p class="step-description">Revisa la informaci├│n antes de crear el documento</p>
            </div>
              ...
              <div class="summary-section">
                ...
                <div class="summary-item">
                  ...
                  <span class="summary-value">{{
                    generateSlug(form.name) || 'Se generar├í autom├íticamente'
                  }}</span>
                </div>
                <div class="summary-item">
...
const wizardSteps = ref([
  ...
  },
  {
    id: 'permissions',
    title: 'Permisos',
    description: 'Configuraci├│n de acceso',
  },
  {
    id: 'summary',
    title: 'Resumen',
    description: 'Revisar informaci├│n',
  },
])
...
const renderedMarkdown = computed(() => {
  console.log('­ƒôØ [MARKDOWN] Computed ejecutado, contenido:', form.value.content ? 'S├¡' : 'No')

  if (!form.value.content || form.value.content.trim() === '') {
    console.log('­ƒôØ [MARKDOWN] No hay contenido para renderizar')
    return '<div class="markdown-empty"><p>No hay contenido para mostrar</p></div>'
  }

  try {
    console.log(
      '­ƒôØ [MARKDOWN] Renderizando contenido:',
    ...
    console.log('­ƒôØ [MARKDOWN] Resultado renderizado:', result.substring(0, 100) + '...')
    console.log('­ƒôØ [MARKDOWN] Longitud del resultado:', result.length)

    return result
  } catch (error) {
    console.error('ÔØî [MARKDOWN] Error renderizando Markdown:', error)
    console.error('ÔØî [MARKDOWN] Contenido que caus├│ el error:', form.value.content)
    return (
      '<div class="markdown-error"><p>ÔØî Error al renderizar el Markdown</p><pre>' +
      error.message +
      '</pre></div>'
    )
  }
...
const isFormValid = computed(() => {
  ...
  console.log('­ƒôä [CONTENT FORM] isFormValid:', {
...
const canProceedToNextStep = computed(() => {
  switch (currentWizardStep.value) {
    case 1:
      // Paso 1: Informaci├│n B├ísica (sin slug, se genera autom├íticamente)
      return (
        form.value.name &&
        form.value.type &&
...
const getTypeLabel = (type) => {
  const types = {
    TYPE_TEXT: '­ƒôä Documento de Texto',
    TYPE_URL: '­ƒöù Enlace/URL',
    TYPE_PDF: '­ƒôï Documento PDF',
  }
...
const getSelectedRolesInfo = () => {
  ...
  // Agregar SUPER_USER siempre si est├í presente
  ...
  switch (field) {
    ...
    case 'content':
      if (!value || value.trim().length === 0) {
        ...
        validationErrors.value.content = 'Debe ser una URL v├ílida'
    ...
    case 'roles':
      if (!value || value.length === 0) {
        ...
        validationErrors.value.roles = 'No puede seleccionar m├ís de 3 roles'
      } else {
...
const validateForm = () => {
  validateField('name')
  validateField('type')
  // No validar slug en modo edici├│n ya que se genera autom├íticamente
  if (!isEditing.value) {
    validateField('slug')
  }
...
// Funci├│n para generar slug autom├íticamente
const generateSlug = (name, makeUnique = false) => {
  ...
  // Si se solicita hacer ├║nico, agregar identificador ├║nico m├ís robusto
  if (makeUnique) {
...
const handlePdfError = () => {
  console.warn('ÔÜá´©Å [PDF] Error cargando PDF:', form.value.content)
}
...
const validatePdfFile = (file) => {
  console.log('­ƒôä [PDF UPLOAD] validatePdfFile llamado con archivo:', file)
  ...
  if (!file) {
    console.log('ÔØî [PDF UPLOAD] No se proporcion├│ archivo')
    validationErrors.value.content = 'No se seleccion├│ ning├║n archivo'
    return false
  }

  console.log('­ƒôä [PDF UPLOAD] Archivo info:', {
    name: file.name,
    type: file.type,
    size: file.size,
  ...
  if (file.type !== 'application/pdf') {
    console.log('ÔØî [PDF UPLOAD] Tipo de archivo inv├ílido:', file.type)
    validationErrors.value.content = 'Solo se permiten archivos PDF'
    return false
  }

  // Validar tama├▒o (10MB m├íximo)
  const maxSize = 10 * 1024 * 1024 // 10MB
  if (file.size > maxSize) {
    console.log('ÔØî [PDF UPLOAD] Archivo muy grande:', formatFileSize(file.size))
  ...
  console.log('­ƒôä [PDF UPLOAD] Archivo asignado - pdfFile.value:', pdfFile.value)
  console.log('­ƒôä [PDF UPLOAD] Contenido del formulario:', form.value.content)

  // Validar el campo content despu├®s de asignar el archivo
  validateField('content')

  console.log('Ô£à [PDF UPLOAD] Archivo PDF v├ílido:', file.name, formatFileSize(file.size))
  return true
}
...
// Funciones espec├¡ficas para PDF
const handlePdfFileChange = (event) => {
  console.log('­ƒôä [PDF UPLOAD] handlePdfFileChange llamado')
  const file = event.target.files[0]
  console.log('­ƒôä [PDF UPLOAD] Archivo seleccionado:', file)
  if (file) {
    validatePdfFile(file)
  } else {
    console.log('­ƒôä [PDF UPLOAD] No se seleccion├│ ning├║n archivo')
  }
}

const removePdfFile = () => {
  console.log('­ƒôä [PDF UPLOAD] removePdfFile llamado')
  pdfFile.value = null
  form.value.content = ''
  if (pdfPreviewUrl.value) {
    URL.revokeObjectURL(pdfPreviewUrl.value)
  ...
  validationErrors.value.content = null
  console.log('­ƒôä [PDF UPLOAD] Archivo PDF removido exitosamente')
}
...
const uploadPdfFile = async (documentId, file) => {
  try {
    console.log('­ƒôä [PDF UPLOAD] Subiendo archivo PDF:', file.name, 'para documento:', documentId)
    ...
    console.log('Ô£à [PDF UPLOAD] Archivo PDF subido exitosamente')
  } catch (error) {
    console.error('ÔØî [PDF UPLOAD] Error subiendo archivo PDF:', error)
    throw error
  }
}
...
// Roles disponibles (mismos que en MenuManagerView)
// SUPER_USER se asigna autom├íticamente a todos los documentos, no es seleccionable
const availableRolesList = [
  ...
  },
  {
    ...
    label: 'Colaborador',
    icon: 'fas fa-users',
    description: 'Acceso a funciones b├ísicas',
  },
]
...
const resetForm = () => {
  form.value = {
    ...
    content: '',
    icon: '',
    roles: ['SUPER_USER'], // SUPER_USER se asigna autom├íticamente a todos los documentos
  }
...
const loadDocumentData = () => {
  if (props.document) {
    console.log(
      '­ƒôä [CONTENT FORM] Documento cargado para edici├│n:',
      props.document?.name || 'Sin nombre',
    )
    console.log('­ƒôä [CONTENT FORM] Roles del documento:', props.document.roles)
    ...
    console.log('­ƒôä [CONTENT FORM] Roles procesados:', cleanRoles)

    form.value = {
      name: props.document.name || '',
      type: props.document.type || '',
      slug: '', // Se genera autom├íticamente en edici├│n
...
const handleButtonClick = () => {
  console.log('­ƒôä [CONTENT FORM] Bot├│n clickeado')
  console.log('­ƒôä [CONTENT FORM] isFormValid:', isFormValid.value)
  console.log('­ƒôä [CONTENT FORM] isSaving:', isSaving.value)
  console.log('­ƒôä [CONTENT FORM] currentWizardStep:', currentWizardStep.value)
  console.log('­ƒôä [CONTENT FORM] wizardSteps.length:', wizardSteps.value.length)

  // Si el formulario es v├ílido y no se est├í guardando, proceder con el env├¡o
  if (isFormValid.value && !isSaving.value) {
    console.log('­ƒôä [CONTENT FORM] Ejecutando handleSubmit directamente')
    handleSubmit()
  } else {
    console.log('­ƒôä [CONTENT FORM] No se puede proceder - Formulario inv├ílido o guardando')
  }
}

const handleSubmit = async () => {
  console.log('­ƒôä [CONTENT FORM] handleSubmit llamado')
  console.log('­ƒôä [CONTENT FORM] Estado del formulario:', {
    name: form.value.name,
    type: form.value.type,
    content: form.value.content,
  ...
  if (Object.keys(validationErrors.value).length > 0) {
    console.log('ÔØî [CONTENT FORM] Errores de validaci├│n:', validationErrors.value)
    return
  }
  ...
  try {
    ...
    if (documentData.type === 'TYPE_URL' && !isValidUrl(documentData.content)) {
      throw new Error('La URL proporcionada no es v├ílida')
    }
    ...
    if (documentData.type === 'TYPE_PDF') {
      console.log('­ƒöì [CONTENT FORM] Validaci├│n PDF - pdfFile.value:', pdfFile.value)
      console.log('­ƒöì [CONTENT FORM] Validaci├│n PDF - pdfFile.name:', pdfFile.value?.name)
      console.log('­ƒöì [CONTENT FORM] Validaci├│n PDF - pdfFile.size:', pdfFile.value?.size)
      ...
      if (!pdfFile.value.name) {
        throw new Error('El archivo PDF seleccionado no es v├ílido')
      }
    }

    console.log('­ƒôä [CONTENT FORM] Datos del documento:', documentData)
    console.log('­ƒôä [CONTENT FORM] Roles espec├¡ficos:', documentData.roles)
    console.log('­ƒôä [CONTENT FORM] Tipo de roles:', typeof documentData.roles)
    console.log('­ƒôä [CONTENT FORM] Es array:', Array.isArray(documentData.roles))
    console.log('­ƒôä [CONTENT FORM] Longitud:', documentData.roles?.length)
    console.log('­ƒôä [CONTENT FORM] Roles del formulario:', form.value.roles)
    console.log(
      '­ƒôä [CONTENT FORM] Roles disponibles:',
    ...
    console.log('­ƒôä [CONTENT FORM] ┬┐Tiene prefijos duplicados?', hasDuplicatePrefix)

    if (hasDuplicatePrefix) {
      console.warn(
        'ÔÜá´©Å [CONTENT FORM] Detectados roles con prefijo ROLE_ duplicado:',
        documentData.roles,
      )
    }
    ...
    if (isEditing.value) {
      console.log('­ƒôä [CONTENT FORM] Actualizando documento existente...')
      await documentService.updateDocument(props.document.id, documentData)
      console.log('Ô£à [CONTENT FORM] Documento actualizado exitosamente')
    } else {
      // Para documentos PDF, usar el endpoint espec├¡fico de creaci├│n con archivo
      console.log('­ƒöì [CONTENT FORM] Debug - Tipo:', documentData.type)
      console.log('­ƒöì [CONTENT FORM] Debug - pdfFile.value:', pdfFile.value)
      console.log(
        '­ƒöì [CONTENT FORM] Debug - Condici├│n PDF:',
        documentData.type === 'TYPE_PDF' && pdfFile.value,
      )

      if (documentData.type === 'TYPE_PDF' && pdfFile.value) {
        console.log('­ƒôä [CONTENT FORM] Creando documento PDF con archivo...')
        const createdDocument = await documentService.createDocumentWithFile(
          documentData,
          pdfFile.value,
        )
        console.log('Ô£à [CONTENT FORM] Documento PDF creado exitosamente')
        createdDocumentId = createdDocument.id
        documentDataWithId.id = createdDocumentId
      } else {
        console.log('­ƒôä [CONTENT FORM] Creando nuevo documento...')
        const createdDocument = await documentService.createDocument(documentData)
        console.log('Ô£à [CONTENT FORM] Documento creado exitosamente')
        createdDocumentId = createdDocument.id
        documentDataWithId.id = createdDocumentId
      }
    }
    ...
    if (isEditing.value && documentData.type === 'TYPE_PDF' && pdfFile.value) {
      console.log('­ƒôä [CONTENT FORM] Subiendo archivo PDF...')
      const documentId = props.document.id
      await uploadPdfFile(documentId, pdfFile.value)
      console.log('Ô£à [CONTENT FORM] Archivo PDF subido exitosamente')
    }

    console.log('­ƒôä [CONTENT FORM] Emitiendo evento saved con ID:', documentDataWithId.id)
    console.log('­ƒôä [CONTENT FORM] Documento completo:', documentDataWithId)

    emit('saved', documentDataWithId)
    handleClose()
  } catch (error) {
    console.error('ÔØî [CONTENT FORM] Error guardando documento:', error)
    console.error('ÔØî [CONTENT FORM] Status:', error.response?.status)
    console.error('ÔØî [CONTENT FORM] Response data:', error.response?.data)

    if (documentData) {
      console.error('ÔØî [CONTENT FORM] Request data:', documentData)
    } else {
      console.error('ÔØî [CONTENT FORM] Request data: No disponible (error en validaci├│n)')
    }
    ...
    if (
      ...
      console.log('­ƒöä [CONTENT FORM] Slug duplicado detectado, regenerando autom├íticamente...')
      ...
      console.log('­ƒôØ [CONTENT FORM] Nuevo slug generado:', newSlug)

      // Mostrar mensaje al usuario
      console.log('Ôä╣´©Å [CONTENT FORM] Slug regenerado autom├íticamente para evitar duplicados')
      ...
      try {
        ...
        if (isEditing.value) {
          ...
          if (newDocumentData.type === 'TYPE_PDF' && pdfFile.value) {
            console.log('­ƒôä [CONTENT FORM] Reintentando creaci├│n de documento PDF con archivo...')
            const createdDocument = await documentService.createDocumentWithFile(
              newDocumentData,
              pdfFile.value,
            )
        ...
        console.log('Ô£à [CONTENT FORM] Documento guardado exitosamente con slug ├║nico')
        ...
        console.log(
          '­ƒôä [CONTENT FORM] Emitiendo evento saved con ID (slug ├║nico):',
          newDocumentDataWithId.id,
        )
        console.log('­ƒôä [CONTENT FORM] Documento completo (slug ├║nico):', newDocumentDataWithId)

        emit('saved', newDocumentDataWithId)
        handleClose()
        return
      } catch (retryError) {
        console.error('ÔØî [CONTENT FORM] Error en reintento:', retryError)
    ...
    if (error.response?.status === 422) {
      console.error('ÔØî [CONTENT FORM] Error 422: Datos no v├ílidos')
      console.error('ÔØî [CONTENT FORM] Detalles del error:', error.response?.data)

      // Si hay errores de validaci├│n espec├¡ficos, mostrarlos
      if (error.response?.data?.errors) {
        console.error('ÔØî [CONTENT FORM] Errores de validaci├│n:', error.response.data.errors)
      }

      // Mostrar mensaje m├ís espec├¡fico
      if (error.response?.data?.message) {
        console.error('ÔØî [CONTENT FORM] Mensaje del servidor:', error.response.data.message)
      }

      // Mostrar campos espec├¡ficos que fallaron
      if (error.response?.data?.validation) {
        console.error('ÔØî [CONTENT FORM] Validaci├│n fallida:', error.response.data.validation)
      }
    } else if (error.response?.status === 400) {
      console.error('ÔØî [CONTENT FORM] Error 400: Solicitud incorrecta')
    } else if (error.response?.status === 401) {
      console.error('ÔØî [CONTENT FORM] Error 401: No autorizado')
    } else if (error.response?.status === 500) {
      console.error('ÔØî [CONTENT FORM] Error 500: Error interno del servidor')
    } else if (error.message) {
      console.error('ÔØî [CONTENT FORM] Error de validaci├│n:', error.message)
    }
...
const handleClose = () => {
  console.log('­ƒôä [CONTENT FORM] Cerrando formulario...')
  console.log('­ƒôä [CONTENT FORM] Estado de edici├│n:', isEditing.value)

  resetForm()
  emit('close')

  console.log('­ƒôä [CONTENT FORM] Formulario cerrado')
}
...
// Watcher para generar slug autom├íticamente cuando cambie el nombre
watch(
  () => form.value.name,
  (newName) => {
...
/* Estilos del Wizard - Id├®nticos a MenuManagerView.vue */
.wizard-header {
...
/* Estilos espec├¡ficos para iconos - Id├®nticos a MenuManagerView.vue */
