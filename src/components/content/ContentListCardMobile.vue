<template>
  <div :class="['cards-wrapper', { 'cards-row': horizontal }]"></div>
  <div
    v-for="doc in items"
    :key="`table-card-${doc.id}`"
    class="rounded-xl border border-slate-200 bg-white p-4 shadow dark:border-slate-700 dark:bg-slate-900 min-w-[300px] flex-shrink-0"
  >
    <div class="flex flex-col gap-3">
      ...
      <div class="flex items-start justify-between gap-3">
        ...
        >
          <i
            :class="
              getDocumentStatus(doc) === 'Activo' ? 'fas fa-check-circle' : 'fas fa-times-circle'
            "
          ></i>
...
.cards-wrapper.cards-row::-webkit-scrollbar {
  height: 8px;
}
.cards-wrapper.cards-row::-webkit-scrollbar-thumb {
  background: rgba(0, 0, 0, 0.08);
  border-radius: 4px;
}
.content-list-card-mobile {
}
</style>
