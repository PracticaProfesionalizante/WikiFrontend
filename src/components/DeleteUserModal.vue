<template>
  <div v-if="show" class="fixed inset-0 z-[1000] grid place-items-center bg-black/60 p-4" @click="handleClose">
    <div class="w-full max-w-[600px] max-h-[90vh] overflow-hidden rounded-2xl border border-slate-200 bg-white text-slate-900 shadow-2xl dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100" @click.stop>
      <!-- Modal Header -->
      <div class="relative rounded-t-2xl bg-gradient-to-tr from-red-600 to-red-500 px-8 py-6 text-white">
        <div class="relative z-10 flex items-start justify-between gap-4">
          ...
          <button @click="handleClose" class="grid h-10 w-10 place-items-center rounded-full bg-white/20 text-white ring-1 ring-white/30 backdrop-blur transition hover:scale-105 disabled:opacity-50" :disabled="isDeleting">
